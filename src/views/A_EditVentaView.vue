<template>
      ...
      <br>
        ...
        <div v-else>
          <div v-for="especializacion in especializaciones" :key="especializacion.id">
            ...
            <!-- Opciones de edición desplegadas debajo de la especialización seleccionada -->
    ...
    <button type="button" @click="addProfesor">Agregar Profesor</button>
                ...
                <div v-for="(clase, index) in clases" :key="index" class="form-group">
                  ...
                  <input v-model="clase.youtubeVideoUrl" placeholder="URL de Video de YouTube" />
  ...
  <input v-model="clase.imagenes[imageIndex]" placeholder="URL de la imagen" />
...
<button type="button" @click="addImagen(index)">Agregar Imagen</button>
  ...
  const clases = ref<
  { id?: string; nombre: string; tematica: string; youtubeVideoUrl?: string; driveLinkClase?: string; pdfClase?: string; imagenes: string[]; texto?: string }[]
>([]);
      ...
      if (especializacion.data) {
        ...
        if (clasesResponse.data) {
          clases.value = clasesResponse.data.map((clase: any) => ({
            ...
            imagenes: clase.imagenes ? clase.imagenes.split(',') : [],
            texto: clase.texto,
...
const removeProfesor = (index: number) => {
  ...
  const addClass = () => {
    clases.value.push({ nombre: '', tematica: '', youtubeVideoUrl: '', imagenes: [], texto: '' });
  };
  ...
  clases.value[index].imagenes.push(''); // Agregar un nuevo input vacío
};

const removeImagen = (index: number, imageIndex: number) => {
  clases.value[index].imagenes.splice(imageIndex, 1); // Eliminar la imagen específica
};
      ...
      for (const clase of clases.value) {
        if (clase.id) {
          await client.models.Clase.update({
            ...
            driveLinkClase: clase.driveLinkClase,
          pdfClase: clase.pdfClase,
          imagenes: JSON.stringify(clase.imagenes),  // Guardar las imágenes como una cadena separada por comas
            texto: clase.texto,
